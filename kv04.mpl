...
path_save := "output/kv04.mpl":

read "load/background.mpl";

read "load/sphere_killing_vectors.mpl";

grdef("KV{a} := KV4sphere{a}");
grdef("KVstar{a} := KV4sphere_star{a}");
...
fprintf(fd, "bracket_integrand_4 := %a;\n", bracket_integrand):
fprintf(fd, "series_integrand_4 := %a;\n", series_integrand):
fprintf(fd, "correction_4:= %a;\n", correction):
fclose(fd):
