...
path_save := "output/kv06.mpl":

read "load/background.mpl";

read "load/sphere_killing_vectors.mpl";

grdef("KV{a} := KV6sphere{a}");
grdef("KVstar{a} := KV6sphere_star{a}");
...
fprintf(fd, "bracket_integrand_6 := %a;\n", bracket_integrand):
fprintf(fd, "series_integrand_6 := %a;\n", series_integrand):
fprintf(fd, "correction_6:= %a;\n", correction):
fclose(fd):
