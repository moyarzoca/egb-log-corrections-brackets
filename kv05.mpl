...
path_save := "output/kv05.mpl":

read "load/background.mpl";

read "load/sphere_killing_vectors.mpl";

grdef("KV{a} := KV5sphere{a}");
grdef("KVstar{a} := KV5sphere_star{a}");
...
fprintf(fd, "bracket_integrand_5 := %a;\n", bracket_integrand):
fprintf(fd, "series_integrand_5 := %a;\n", series_integrand):
fprintf(fd, "correction_5:= %a;\n", correction):
fclose(fd):
