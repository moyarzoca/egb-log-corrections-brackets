...
path_save := "output/kv03.mpl":

read "load/background.mpl";

read "load/sphere_killing_vectors.mpl";

grdef("KV{a} := KV3sphere{a}");
grdef("KVstar{a} := KV3sphere_star{a}");
...
fprintf(fd, "bracket_integrand_3 := %a;\n", bracket_integrand):
fprintf(fd, "series_integrand_3 := %a;\n", series_integrand):
fprintf(fd, "correction_3:= %a;\n", correction):
fclose(fd):
