...
path_save := "output/kv01.mpl":

read "load/background.mpl";

read "load/sphere_killing_vectors.mpl";

grdef("KV{a} := KV1sphere{a}");
grdef("KVstar{a} := KV1sphere_star{a}");
...
fprintf(fd, "bracket_integrand_1 := %a;\n", bracket_integrand):
fprintf(fd, "series_integrand_1 := %a;\n", series_integrand):
fprintf(fd, "correction_1 := %a;\n", correction):
fclose(fd):
